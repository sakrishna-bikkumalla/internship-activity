...
def mock_streamlit():
    with patch("internship_activity_tracker.ui.batch.st") as mock_st:
        ...
        mock_st.columns = MagicMock(
            side_effect=lambda n: [MagicMock() for _ in range(n if isinstance(n, int) else len(n))]
        )
...
class TestRenderBatchAnalyticsUI:
    def test_render_success(self, mock_client, mock_streamlit):
        ...
        with (
            patch(
                "internship_activity_tracker.ui.batch.get_all_members_with_teams",
                return_value=[{"name": "User 1", "gitlab_username": "user1"}],
            ),
