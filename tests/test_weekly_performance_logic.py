...
@patch("internship_activity_tracker.ui.weekly_performance.st.markdown")
...
def test_render_activity_slots_yellow_streak(mock_markdown):
    ...
    weekly_performance._render_activity_slots(active_hours=[9, 14], slots=[9, 10, 11, 12, 13, 14], use_strict_mode=True)
    html = mock_markdown.call_args[0][0]
...
def test_render_activity_slots_total_idle(mock_markdown):
    ...
    weekly_performance._render_activity_slots(active_hours=[], slots=[9, 10], use_strict_mode=True)
    html = mock_markdown.call_args[0][0]
...
def test_init_state():
    state = {}
    with patch("internship_activity_tracker.ui.weekly_performance.st.session_state", state):
