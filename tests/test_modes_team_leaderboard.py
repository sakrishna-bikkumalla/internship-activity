...
def test_build_ranking_rows():
    team_data = {
        "Team A": ({"project_name": "P1"}, [], {"Team Score": 100, "Total Commits": 50, "MR Merged": 10, "Issues Closed": 5}),
        "Team B": ({"project_name": "P2"}, [], {"Team Score": 200, "Total Commits": 100, "MR Merged": 20, "Issues Closed": 10})
    }
...
def test_build_individual_rows_and_badges():
    team_data = {
        "Team A": (
            {},
            [
                {"Username": "u1", "Status": "Success", "Score": 100, "Total Commits": 50, "MR Merged": 10, "Issues Closed": 5},
                {"Username": "u2", "Status": "Success", "Score": 50, "Total Commits": 20, "MR Merged": 5, "Issues Closed": 2}
            ],
            {}
        )
    }
...
def test_validate_json_teams():
    with patch("streamlit.session_state", {"teams": []}):
        ...
        valid_json = {
            "teams": [
                {
                    "team_name": "Team 1", "project_name": "P1",
                    "members": [{"name": "M1", "username": "u1"}]
                }
            ]
        }
